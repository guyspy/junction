 ...
 * a single self-contained HTML file. Keyboard focus, reduced motion, and small screens
 * are first-class (school Chromebooks; WCAG 2.1 AA posture).
 */
...
body {
  ...
  background:
    radial-gradient(1100px 600px at 50% -10%, #2a8a5c 0%, var(--felt) 55%, var(--felt-edge) 100%);
  color: #f4f7f4;
  min-height: 100vh;
}
...
.jx-status {
  font-size: 16px; font-weight: 650; margin: 6px 0 14px; min-height: 24px;
}
...
.jx-zone.mine { border-color: rgba(255, 209, 102, 0.45); }
.jx-zone h2 {
...
button.jx-card.playable::after {
  ...
  border: 2.5px solid var(--accent); box-shadow: 0 0 14px rgba(255, 209, 102, 0.55);
  pointer-events: none;
}
...
.jx-button {
  ...
  border: 0; background: var(--accent); color: #3a2c00; cursor: pointer;
  box-shadow: 0 3px 0 rgba(0,0,0,0.25);
}
.jx-button:hover { filter: brightness(1.06); }
.jx-button:focus-visible { outline: 3px solid #fff; outline-offset: 2px; }
.jx-button:active { transform: translateY(2px); box-shadow: 0 1px 0 rgba(0,0,0,0.25); }
...
@media (prefers-reduced-motion: reduce) {
  .jx-card, .jx-card-inner { transition: none !important; }
}
`;
